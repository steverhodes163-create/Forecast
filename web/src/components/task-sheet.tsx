...
import {
  ...
  setTaskResourcesAction,
  markTaskDoneAction,
  deleteTaskAction,
  createBlankTaskAction,
} from "@/app/actions/tasks";
import { formatPredecessors, formatResources } from "@/lib/task-shorthand";
...
export function TaskSheet({ projectId, tasks }: { projectId: number; tasks: GanttTask[] }) {
  ...
  async function commitResources(taskId: number, raw: string) {
    const key = `res-${taskId}`;
    const result = await setTaskResourcesAction({ taskId, projectId, raw });
    if ("error" in result) {
      setError(key, result.error);
      return;
    }
      ...
      <table className="w-full min-w-[900px] border-collapse text-sm">
        <thead>
          <tr className="border-b border-slate-200 text-left text-xs font-medium uppercase tracking-wide text-slate-400">
            ...
            <th className="w-32 px-2 py-1.5">Predecessors</th>
            <th className="w-52 px-2 py-1.5">Resources</th>
        ...
        <tbody>
          {tasks.map((t) => (
            <tr key={t.id} className={`border-b border-slate-50 ${(t.id in pendingDone ? pendingDone[t.id] : !!t.completedAt) ? "bg-slate-50" : ""}`}>
              ...
              <td className="px-1 py-0.5">
                <input
                  type="text"
                  defaultValue={formatResources(t.assignees.map((a) => ({ name: a.name, pct: Math.round(a.fte * 100) })))}
                  onBlur={(e) => commitResources(t.id, e.currentTarget.value)}
                  placeholder="e.g. Alex Whitfield[50%]"
                  className={`${cellInputClass} ${cellError[`res-${t.id}`] ? "border-red-400" : ""}`}
                  title={cellError[`res-${t.id}`]}
                />
              </td>
          ...
          <tr>
            ...
            <td className="px-1 py-0.5" colSpan={7}>
              <input
                type="text"
                value={newTaskName}

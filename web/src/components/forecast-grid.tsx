...
import type { GridMonthColumn, GridTeam } from "@/lib/forecast-grid";
...
export function ForecastGrid({
  ...
  function hoursFor(employeeId: number, dateKey: number, baseline: number): number {
    const key = cellKey(employeeId, dateKey);
    return key in edits ? edits[key] : baseline;
  }

  function employeeWeekValue(employeeId: number, dateKey: number, baseline: number, standardWeeklyHours: number): number {
    const hours = hoursFor(employeeId, dateKey, baseline);
    return unit === "fte" ? fteFromHours(hours, standardWeeklyHours) : hours;
  }

  function teamWeekValue(team: GridTeam, dateKey: number): number {
    return team.employees.reduce(
      (sum, e) => sum + employeeWeekValue(e.employeeId, dateKey, e.hoursByDateKey[dateKey] ?? 0, e.standardWeeklyHours),
      0
    );
  }
              ...
              <Fragment key={team.teamId}>
                ...
                {team.employees.map((employee) => (
                  <tr key={employee.employeeId}>
                    <td className="py-1 pl-6 pr-2 text-slate-600">{employee.name}</td>
                    {months.map((m) =>
                      expanded.has(m.monthKey) ? (
                        m.weeks.map((w) => {
                          const baseline = employee.hoursByDateKey[w.dateKey] ?? 0;
                          const value = employeeWeekValue(employee.employeeId, w.dateKey, baseline, employee.standardWeeklyHours);
                          ...
                          return (
                            <td key={w.dateKey} className="border-l border-slate-100 px-1 py-0.5">
                              <input
                                ...
                                min={0}
                                disabled={disabled}
                                defaultValue={formatValue(value, unit)}
                        ...
                        <td key={m.monthKey} className="border-l border-slate-100 px-2 py-1.5 text-right tabular-nums text-slate-500">
                          {formatValue(
                            m.weeks.reduce(
                              (sum, w) => sum + employeeWeekValue(employee.employeeId, w.dateKey, employee.hoursByDateKey[w.dateKey] ?? 0, employee.standardWeeklyHours),
                              0
                            ),
                            unit
                          )}
